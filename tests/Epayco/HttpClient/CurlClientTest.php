...
/**
 * @internal
 * @covers \Epayco\HttpClient\CurlClient
 */
final class CurlClientTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $this->origMaxNetworkRetries = \Epayco\Epayco::getMaxNetworkRetries();
        $this->origMaxNetworkRetryDelay = \Epayco\Epayco::getMaxNetworkRetryDelay();
        $this->origInitialNetworkRetryDelay = \Epayco\Epayco::getInitialNetworkRetryDelay();
    }
    ...
    {
        $epaycoReflector = new \ReflectionClass('\Epayco\Epayco');

        $this->maxNetworkRetryDelayProperty = $epaycoReflector->getProperty('maxNetworkRetryDelay');
        $this->maxNetworkRetryDelayProperty->setAccessible(true);

        $this->initialNetworkRetryDelayProperty = $epaycoReflector->getProperty('initialNetworkRetryDelay');
        $this->initialNetworkRetryDelayProperty->setAccessible(true);

        $curlClientReflector = new \ReflectionClass('\Epayco\HttpClient\CurlClient');
    ...
    {
        \Epayco\Epayco::setMaxNetworkRetries($this->origMaxNetworkRetries);
    ...
    {
        $fakeRandomGenerator = $this->createMock('\Epayco\Util\RandomGenerator');
    ...
    public function testDefaultOptions()
    {
        ...
        $this->expectException('Epayco\Exception\UnexpectedValueException');
    ...
    public function testShouldRetryOnEpaycoShouldRetryTrue()
    {
        \Epayco\Epayco::setMaxNetworkRetries(2);
        ...
        static::assertTrue($this->shouldRetryMethod->invoke($curlClient, 0, 400, ['epayco-should-retry' => 'true'], 0));
    }

    public function testShouldNotRetryOnEpaycoShouldRetryFalse()
    {
        \Epayco\Epayco::setMaxNetworkRetries(2);
        ...
        static::assertTrue($this->shouldRetryMethod->invoke($curlClient, 0, 500, [], 0));
        static::assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 500, ['epayco-should-retry' => 'false'], 0));
    }
    ...
    public function testShouldNotRetryAtMaximumCount()
    {
        ...
        static::assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 0, [], \Epayco\Epayco::getMaxNetworkRetries()));
    }
    ...
    public function testSleepTimeShouldGrowExponentially()
    {
        ...
        static::assertSame(
            \Epayco\Epayco::getInitialNetworkRetryDelay() * 1,
            $this->sleepTimeMethod->invoke($curlClient, 1, [])
        );
        static::assertSame(
            \Epayco\Epayco::getInitialNetworkRetryDelay() * 2,
            $this->sleepTimeMethod->invoke($curlClient, 2, [])
        );
        static::assertSame(
            \Epayco\Epayco::getInitialNetworkRetryDelay() * 4,
            $this->sleepTimeMethod->invoke($curlClient, 3, [])
        );
        static::assertSame(
            \Epayco\Epayco::getInitialNetworkRetryDelay() * 8,
    ...
    public function testSleepTimeShouldAddSomeRandomness()
    {
        ...
        $baseValue = \Epayco\Epayco::getInitialNetworkRetryDelay() * (0.5 * (1 + $randomValue));

        // the initial value cannot be smaller than the base,
        // so the randomness is ignored
        static::assertSame(\Epayco\Epayco::getInitialNetworkRetryDelay(), $this->sleepTimeMethod->invoke($curlClient, 1, []));
    ...
    public function testSetRequestStatusCallback()
    {
        try {
            ...
            \Epayco\ApiRequestor::setHttpClient(null);
        }
    }
}
