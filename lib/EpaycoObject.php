 ...
 * Class EpaycoObject.
 */
class EpaycoObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToEpaycoObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_transientValues) && $this->_transientValues->includes($k)) {
            ...
            $message = "Epayco Notice: Undefined property of {$class} instance: {$k}. "
                    . "HINT: The {$k} attribute was set in the past, however. "
                    . 'It was then wiped when refreshing the object '
                    . "with the result returned by Epayco's API, "
                    . 'probably as a result of a save(). The attributes currently '
                    . "available on this object are: {$attrs}";
            Epayco::getLogger()->error($message);

            return $nullval;
        }
        $class = static::class;
        Epayco::getLogger()->error("Epayco Notice: Undefined property of {$class} instance: {$k}");
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof EpaycoObject) {
    ...
    {
        foreach ($values as $k => $v) {
            // Special-case metadata to always be cast as a EpaycoObject
            // This is necessary in case metadata is empty, as PHP arrays do
            // not differentiate between lists and hashes, and we consider
            // empty arrays to be lists.
            if (('metadata' === $k) && (\is_array($v))) {
                $this->_values[$k] = EpaycoObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToEpaycoObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            //   3. Its value is a EpaycoObject. A EpaycoObject may contain modified
            //      values within in that its parent EpaycoObject doesn't know about.
            //
            $original = \array_key_exists($k, $this->_originalValues) ? $this->_originalValues[$k] : null;
            $unsaved = $this->_unsavedValues->includes($k);
            if ($force || $unsaved || $v instanceof EpaycoObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (Util\Util::isList($value)) {
                ...
                return Util\Util::convertToEpaycoObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof EpaycoObject) {
    ...
    /**
     * Returns a pretty JSON representation of the Epayco object.
     *
     * @return string the JSON representation of the Epayco object
     */
    ...
    /**
     * Sets all keys within the EpaycoObject as unsaved so that they will be
     * included with an update when `serializeParameters` is called. This
     * method is also recursive, so any EpaycoObjects contained as values or
    ...
    protected static function deepCopy($obj)
    {
        ...
        if ($obj instanceof EpaycoObject) {
    ...
    public static function emptyValues($obj)
    {
        if (\is_array($obj)) {
            $values = $obj;
        } elseif ($obj instanceof EpaycoObject) {
            $values = $obj->_values;
        } else {
            throw new Exception\InvalidArgumentException(
    ...
    /**
     * @return null|ApiResponse The last response from the Epayco API
     */
    ...
    /**
     * Sets the last response from the Epayco API.
     *
