...
class File extends ApiResource
{
    ...
    // versions, only `file` is used, but since epayco-php may be used with
     ...
     *
     * @return \Epayco\File the created file
     */
    public static function create($params = null, $opts = null)
    {
        $opts = \Epayco\Util\RequestOptions::parse($opts);
        if (null === $opts->apiBase) {
            $opts->apiBase = Epayco::$apiUploadBase;
        }
        // Manually flatten params, otherwise curl's multipart encoder will
        // choke on nested arrays.
        $flatParams = \array_column(\Epayco\Util\Util::flattenParams($params), 1, 0);
